...
public class Promote extends TeamChatSubCommand {
    public Promote(String command) {
        super(command);
        setRequireTeam(true);
    }
    ...
    public void execute(Player player, List<String> args) {
        ...
        if (target.getUniqueId().toString().equalsIgnoreCase(player.getUniqueId().toString())) {
            PlayerUtil.sendMessage(player, Main.getPrefix() + " §cYou are not allowed to promote yourself!");
            return;
        }
        ...
        if (team.getTeamPerms().getCanPromoteAndDemote().getId() > targetMember.getRole().getId()) {
            PlayerUtil.sendMessage(player, Main.getPrefix() + " §cYou are not allowed to promote players!");
            return;
        }

        if (targetMember.getRole().getId() >= teamMember.getRole().getId()) {
            PlayerUtil.sendMessage(player, Main.getPrefix() + " §cYou are not allowed to promote "+target.getName()+"!");
            return;
        }
        targetMember.setRole(TeamRole.byID(targetMember.getRole().getId()+1));
        PlayerUtil.sendMessage(player, Main.getPrefix() + " §cYou have promoted "+target.getName()+"!");
    }
}
